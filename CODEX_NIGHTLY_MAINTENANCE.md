...
- For an explicit single multipack such as `2 x 180 g` or `4 x 175 g`, calculate unit price from the total package quantity and display both the per-item size and total. If the same offer is a cross-size assortment such as `2x75 ml/500 ml`, publish no guessed unit price.
- If an offer explicitly lets the shopper choose ribeye/entrecôte, include it in the ribeye comparison aisle even when striploin or rump steak is another choice. Preserve all choices in the Chinese name and explanation.
