...
const atlantaGroup = (nameZh, noteZh = '同一商品主体归组；尺寸和包装保留原名，价格比较仍需结合净重。', comparable = true) => ({ nameZh, noteZh, comparable });
export const ATLANTA_COMPARISON_GROUPS = {
  ...
  produce_broccoli: atlantaGroup('西兰花'), produce_cauliflower: atlantaGroup('花椰菜'), produce_cruciferous: atlantaGroup('十字花科蔬菜（旧分类）', '兼容旧记录，不计算跨品种最低价。', false), produce_grapes: atlantaGroup('葡萄'), produce_plums: atlantaGroup('李子'),
...
function atlantaComparisonGroup(name, categoryId) {
  ...
  const rules = {
    produce: [
      ...
      ['produce_peppers', /peppers?/], ['produce_cucumbers', /cucumbers?/], ['produce_broccoli', /broccoli/], ['produce_cauliflower', /cauliflower/],
    ...
    meat: [
      ...
      ['meat_meatballs', /beef meatballs?|gourmet beef meatballs?/], ['meat_lamb', /lamb/], ['meat_turkey_deli', /turkey breast/], ['meat_ham_deli', /ham/], ['meat_pepperoni', /pepperoni/], ['meat_sticks', /meat sticks?/], ['meat_sausage', /sausage/],
    ],
    seafood: [['seafood_salmon_burgers', /salmon burgers?/], ['seafood_salmon', /salmon|sockeye/], ['seafood_shrimp', /shrimp|prawns?/], ['seafood_tuna_canned', /starkist|canned tuna/], ['seafood_mixed_fish_steaks', /tuna.*swordfish|swordfish.*tuna/], ['seafood_tuna_fresh', /tuna/]],
