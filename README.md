...
申请第三方数据展示许可期间，公开 Pages 使用 `site-mode.json` 中的 `permission-review` 模式。该模式只发布白名单静态文件和 `data/review_demo.json` 中的虚构演示数据；定时工作流不会刷新或发布真实促销内容。

取得覆盖所需数据字段、图片、缓存、更新频率和署名方式的书面许可后，可一键切换回正常模式：
...
恢复必须通过测试和人工确认，并将 `site-mode.json` 的变更提交、合并至 `main` 后才会部署。需要再次暂停时运行 `npm run site:review`。未取得明确许可时，不得切回 `live`。
