...
test('published data contains no private home-base fields, secrets, or home wording', async () => {
  const [data, atlanta, descriptionCache, descriptionPending, productTaxonomy, taxonomyPending] = await Promise.all([
  ...
  assert.doesNotMatch(JSON.stringify({ descriptionCache, descriptionPending, productTaxonomy, taxonomyPending }), /sk-[A-Za-z0-9_-]{20,}|OPENAI_API_KEY|api[_-]?key/i);
